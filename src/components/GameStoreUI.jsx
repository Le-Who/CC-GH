      ...
      >
        ...
        <div className="flex justify-between items-center p-4 border-b border-border bg-background/50">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={onClose}
              className="text-textDim hover:text-white p-1 ml-2"
            >
              ✕
            </button>
          </div>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full whitespace-nowrap transition-colors ${
                  isActive
                    ...
                    : "bg-surfaceHover text-textDim hover:text-white"
                }`}
              >
                {tab}
              </button>
  ...
  return (
    <div className="flex flex-col items-center h-full">
          ...
          >
            ...
            <ul className="space-y-1">
              <li className="flex justify-between">
                <span className="text-gray-400">Common (Decorations)</span>{" "}
                <span>60.0%</span>
              </li>
              <li className="flex justify-between">
                <span className="text-blue-400">Rare (Pets)</span>{" "}
                <span>30.0%</span>
              </li>
              <li className="flex justify-between">
                <span className="text-purple-400">Epic (Auras)</span>{" "}
                <span>9.0%</span>
              </li>
              <li className="flex justify-between">
                <span className="text-gold">Legendary (Golden Skins)</span>{" "}
                <span>1.0%</span>
              </li>
            </ul>
            <p className="text-xs text-textDim mt-3 italic">
              Pity System: Legendary guaranteed within 50 pulls.
            </p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex gap-4 w-full mt-auto">
        ...
        <button className="flex-2 py-4 bg-primary text-white rounded-xl font-bold flex flex-col items-center justify-center shadow-[0_0_20px_rgba(167,139,250,0.4)] hover:opacity-90 transition-opacity w-2/3">
    ...
    <div className="flex flex-col gap-6">
      <div className="relative rounded-2xl overflow-hidden p-6 border border-gold/50 bg-[#1A1A10]">
        ...
        <div className="relative z-10">
          <h3 className="text-2xl font-black italic text-gold tracking-widest uppercase transform -skew-x-6">
            VIP Cozy Pass
          </h3>
          <p className="text-white/80 mt-2">
            Unlock the Golden Shiba pet variant + 500 immediate Energy!
          </p>
          <button className="mt-4 px-6 py-2 bg-gold text-black font-bold rounded-full hover:bg-yellow-400">
            $4.99 / Season
          </button>
        </div>
      </div>
...
function BundlesTab() {
  return (
    <div className="text-center text-textDim mt-12">
      No active weekend bundles right now. Check back later!
    </div>
  );
}
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div className="p-4 rounded-xl border border-border bg-surfaceHover flex flex-col items-center text-center">
        ...
        <button className="mt-3 w-full py-2 bg-primary text-white rounded-lg hover:bg-opacity-90">
          $0.99
        </button>
      </div>
      <div className="p-4 rounded-xl border border-gold border-opacity-50 bg-[#1A1A10] flex flex-col items-center text-center relative overflow-hidden">
        ...
        <button className="mt-3 w-full py-2 bg-gold text-black font-bold rounded-lg hover:bg-yellow-400">
          $4.99
        </button>
      </div>
    </div>
