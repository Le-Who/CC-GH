...
|---|---|---|
| Garden Shelf `plant-detail` | Title/meta are cramped inside the hanging plaque; hint text crosses the income panel; action labels and evolve price sit too low near frame decoration. | Patched CSS lane positions and text sizing in `garden-shelf.css`. |
...
| Garden Shelf `quests` | Quest title/body/reward/progress/action lanes visually collide; body text is hard to read at mobile width. | Patched quest card column distribution, font sizes, and clamps in `garden-shelf.css`. |
