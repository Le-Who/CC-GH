...
- Cozy Yard: mixed-pet idle collector replacing the old Pet Room tab. Players use a standalone mobile-style yard HUD, open all food/goodies/shop/petbook/album/gift/settings screens inside the playfield, freely place and move every interactive goodie by yard coordinates, wait server-simulated classic-hour visitor windows, watch pets enter the yard and interact with item-specific activity anchors and pose-specific visitor art, collect gifts and mementos, repair worn goodies, start with Morning Meadow plus Moon Garden backgrounds, buy Tea House from the shop background section, and configure a helper home companion with no real-money paths. Goodie metadata distinguishes layable surfaces from movement blockers: visitors can settle into stationary poses on layables, while non-layable goodies become simple route obstacles. Yard actions use a durable client outbox with entity-level pending visuals so weak connections retry silently instead of showing tap-then-rollback behavior.
